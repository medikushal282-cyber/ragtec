...
// Adapted Tailwind classes for RAGSEC theme without requiring shadcn css variables
const PANEL_BASE =
  "mx-auto flex h-16 items-end gap-3 rounded-2xl border border-white/10 bg-black/70 px-3 pb-3 shadow-[0_0_30px_rgba(0,0,0,0.5)] backdrop-blur-md";

const ITEM_BASE =
  "group relative flex aspect-square items-center justify-center rounded-xl bg-white/5 border border-white/5 text-white/70 hover:text-white hover:bg-white/10 hover:border-white/20 transition-colors shadow-sm";
        ...
        <AnimatePresence>
          {label ? (
            <motion.span
              ...
              exit={{ opacity: 0, y: 2 }}
              className="pointer-events-none absolute -top-9 left-1/2 hidden -translate-x-1/2 whitespace-nowrap rounded-md bg-white/10 border border-white/10 px-2 py-1 text-xs font-medium text-white shadow group-hover:block backdrop-blur-md"
            >
              {label}
            </motion.span>
          ) : null}
